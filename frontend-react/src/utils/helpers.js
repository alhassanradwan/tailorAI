...
// /** Chat history storage (per-user, localStorage) */
// export const ChatHistory = {
//   _key(email) {
//     return `adaptiveai_chat_history_${email || 'guest'}`;
//   },

//   getSessions(email) {
//     const stored = localStorage.getItem(this._key(email));
//     return stored ? JSON.parse(stored) : [];
//   },

//   saveSession(email, session) {
//     const sessions = this.getSessions(email);
//     const idx = sessions.findIndex((s) => s.id === session.id);
//     if (idx >= 0) sessions[idx] = session;
//     else sessions.unshift(session);
//     localStorage.setItem(this._key(email), JSON.stringify(sessions.slice(0, 50)));
...
//   addMessage(email, sessionId, message) {
//     const sessions = this.getSessions(email);
...
//     this.saveSession(email, session);
//     return session;
//   },

//   deleteSession(email, sessionId) {
//     const sessions = this.getSessions(email).filter((s) => s.id !== sessionId);
//     localStorage.setItem(this._key(email), JSON.stringify(sessions));
    
//     // Notify components of update
//     window.dispatchEvent(new Event('chatHistoryUpdated'));
//   },

//   getSession(email, sessionId) {
//     return this.getSessions(email).find((s) => s.id === sessionId) || null;
//   },

//   renameSession(email, sessionId, newTitle) {
//     const sessions = this.getSessions(email);
//     const session = sessions.find((s) => s.id === sessionId);
//     if (session) {
